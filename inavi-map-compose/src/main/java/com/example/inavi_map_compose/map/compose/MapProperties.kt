 ...
 * @param minZoom 지도의 최소 줌 레벨을 지정합니다. 기본값은 [InvMapConstants.MinZoom]입니다.
 * @param maxZoom 지도의 최대 줌 레벨을 지정합니다. 기본값은 [InvMapConstants.MaxZoom]입니다.
 * @param maxTilt 지도의 최대 기울기 각도를 지정합니다. 기본값은 [InvMapConstants.DefaultMaxTilt]입니다.
 * @param defaultCameraAnimationDuration 카메라 이동 애니메이션의 기본 지속 시간을 지정합니다.
 * 기본값은 [InvMapConstants.DefaultCameraAnimationDuration]입니다.
 ...
 * [InaviMapCompose]의 onSymbolClick이 호출되지 않습니다.
 ...
 * 기본값은 [InvMapConstants.DefaultIndoorFocusRadius]입니다.
 ...
 * 기본값은 [InvMapConstants.DefaultBackgroundColorLight]입니다.
 ...
 * 기본값은 [InvMapConstants.DefaultBackgroundDrawableLight]입니다.
...
public data class MapProperties(
    ...
    public val minZoom: Double = InvConstants.MINIMUM_ZOOM,
    public val maxZoom: Double = InvConstants.MAXIMUM_ZOOM,
    public val maxTilt: Double = InvConstants.MAXIMUM_TILT,
    ...
    public val userTrackingMode: UserTrackingMode = UserTrackingMode.None,
) {
    init {
 ...
 */
@Immutable
public enum class MapType(val value: com.inavi.mapsdk.maps.InaviMap.MapType) {
    /**
     * 일반 지도.
     */
    Normal(com.inavi.mapsdk.maps.InaviMap.MapType.Normal),

    /**
     * 위성 지도.
     */
    Satellite(com.inavi.mapsdk.maps.InaviMap.MapType.Satellite),

    /**
     * 위성 지도(겹쳐보기).
     */
    Hybrid(com.inavi.mapsdk.maps.InaviMap.MapType.Hybrid),
}
 ...
 */
@Immutable
public enum class UserTrackingMode(val value: com.inavi.mapsdk.maps.UserTrackingMode) {
    /**
     * 위치 추적을 사용하지 않는 모드.
     */
    None(com.inavi.mapsdk.maps.UserTrackingMode.None),

    /**
     * 위치는 추적하지만 지도는 움직이지 않는 모드.
     */
    NoTracking(com.inavi.mapsdk.maps.UserTrackingMode.NoTracking),

    /**
     * 위치를 추적하면서 카메라도 따라 움직이는 모드.
     * API나 제스처를 사용해 지도를 임의로 움직일 경우 모드가 [NoFollow]로 바뀝니다.
     */
    Tracking(com.inavi.mapsdk.maps.UserTrackingMode.Tracking),

    /**
     * 위치를 추적하면서 카메라의 좌표와 베어링도 따라 움직이는 모드.
     * API나 제스처를 사용해 지도를 임의로 움직일 경우 모드가 [NoFollow]로 바뀝니다.
     */
    TrackingCompass(com.inavi.mapsdk.maps.UserTrackingMode.TrackingCompass)
}