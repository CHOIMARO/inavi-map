 ...
 * 기본값은 [InvMapConstants.DefaultPickTolerance]입니다.
 ...
 * 기본값은 [InvMapConstants.DefaultScrollGesturesFriction]입니다.
 * @param zoomGesturesFriction 줌 제스처의 마찰 계수를 0~1로 지정합니다. 계수가 클수록 마찰이 강해집니다.
 * 기본값은 [InvMapConstants.DefaultZoomGesturesFriction]입니다.
 * @param rotateGesturesFriction 회전 제스처의 마찰 계수를 0~1로 지정합니다. 계수가 클수록 마찰이 강해집니다.
 * 기본값은 [InvMapConstants.DefaultRotateGesturesFriction]입니다.
 ...
 * 기본값은 false입니다.
 * @param isLogoClickEnabled 아이나비 로고 클릭을 활성화할지 여부를 지정합니다.
 * 활성화하면 아이나비 로고 클릭시 범례, 법적 공지, 오픈소스 라이선스를 보여주는 다이얼로그가 열립니다.
 * 이 옵션을 false로 지정하는 앱은 반드시 앱 내에 아이나비 지도 SDK의 법적 공지 액티비티([LegalNoticeActivity]) 및
 * 오픈소스 라이선스 액티비티([OpenSourceLicenseActivity])로 연결되는 메뉴를 만들어야 합니다.
 * 기본값은 true입니다.
 * @param logoGravity 아이나비 로고의 그래비티를 지정합니다.
 * @param logoMargin 아이나비 로고의 마진을 지정합니다.
 */
